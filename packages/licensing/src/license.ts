...
export function toPublicKey(key: KeyInput): KeyObject {
  if (typeof key === 'object' && !(key instanceof Uint8Array)) return key;
  if (isPem(key)) return createPublicKey(key);
...
export function toPrivateKey(key: KeyInput): KeyObject {
  if (typeof key === 'object' && !(key instanceof Uint8Array)) return key;
  if (isPem(key)) return createPrivateKey(key);
  const raw = rawBytes(key);
...
export function assessLicense(
  ...
  const wall = context.now ?? Date.now();
  ...
  const now = context.clockFloor !== undefined ? Math.max(wall, context.clockFloor) : wall;
  const buildDate = context.buildDate ?? now;
  const buildMajor = context.buildMajor ?? 0;
  ...
  const graceMs = (context.graceDays ?? SUBSCRIPTION_GRACE_DAYS) * DAY_MS;
